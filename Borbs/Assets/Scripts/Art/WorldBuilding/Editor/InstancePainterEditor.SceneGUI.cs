    ...
    public partial class InstancePainterEditor : Editor
    {
        void OnSceneGUI()
        {
            ...
            if (Physics.Raycast(ray , out hit , float.MaxValue , ip.layerMask))
            {
                ...
                Handles.color = Color.white * 0.1f;
        ...
        private void OverlapCapsule(Vector3 top , Vector3 bottom , float brushRadius , LayerMask layerMask)
        {
            ...
            if (ip.collisionTest == InstancePainter.CollisionTest.Position)
            {
                foreach (Transform c in ip.rootTransform.transform)
                {
                    if (c.transform.position.DistanceFlatZ(bottom) < brushRadius)
                    {
                        overlaps.Add(c.gameObject.GetRendererBounds());
                        overlappedGameObjects.Add(c.gameObject);
                    }
