...
namespace Gamekit3D.WorldBuilding
{
    ...
    public partial class InstancePainterEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            {
                variations = ip.SelectedPrefab.GetComponent<SetDressingPrefab>();
        ...
        void CreateNewStamp()
        {
            ...
            for (var i = 0 ; i < count ; i++)
            {
                ...
                GameObject dummy;
                if (variations != null)
                    dummy = PrefabUtility.InstantiatePrefab(variations.SetData.gameObjects[Random.Range(0 , variations.SetData.gameObjects.Count)]) as GameObject;
                else
                ...
                if (variations != null)
                    child.transform.localScale = Vector3.one * Mathf.Lerp(variations.SetData.minScale , variations.SetData.maxScale , Random.value);
                dummy.transform.localPosition = Vector3.zero;
                dummy.transform.localRotation = Quaternion.identity;
                // fixes the lag issue
                dummy.isStatic = false;
                foreach (Transform putter in dummy.transform)
                {
                    putter.gameObject.isStatic = false;
                }
            }
        ...
        void PerformStamp()
        {
            var removeVariations = ip.SelectedPrefab.GetComponent<SetDressingPrefab>() != null;
            for (var i = 0 ; i < stamp.transform.childCount ; i++)
            {
                ...
                if (dummy.gameObject.activeSelf)
                {
                    ...
                    if (removeVariations) DestroyImmediate(g.GetComponent<SetDressingPrefab>());
                ...
                dummy.gameObject.isStatic = true;
                foreach (Transform putter in dummy.transform)
                {
                    putter.gameObject.isStatic = true;
                }
            }
